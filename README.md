# Dashboard Incluidos #

# 1. Dashboard de Ações #
**Indicadores e visualizações sobre desempenho de ações:**
* Total Corrente
* Total Investtido
* Percentual de Retorno

# 2.Dashboard Controle de Salários #
**Controle e análise de salários por setor:** 
* Salário por Departamento
* Custo Total de Salários
* Evolução 
* Gastos com Adiantamentos, IPRF, INSS, FGTS e Salários.

# 3. Dashboard Loja Ecommerce #
**Análise de Desempenho das vendas:**
* Total Vendas por Categoria
* Vendas por Vendedor
* Vendas por Produtos
* Vendas por Cliente

# 4. Dashboard de OLrçamento #
**Indicadores de Performance e Evolução:** 
*  Performance e KPI
* Percentual de Consumo 
* Ranking por Setor e Centro de Custo
* Evolução de Realizado x Orçado Mês a Mês

# 5. Dashboard de Vendas Completo #
**Análise Detalhada das Vendas**
* Total Vendas
* Performance
* Share Categoria
* Top 5 Clientes, Produtos e Vendedores
* KPI de Meta
* Evolução de Vendas por Categoria

# 6. Dashboard Vendas Loja #
**Análise sobre as vendas:** 
* Vendas por Estado
* Vendas por Cliente
* Custo, Quantidade de Vendas e Clientes e Total de Vendas

# 7.Dashboard Loja ABC #
**Análise Geral:**
* Vendas por Estado
* Vendas por Cliente
* Vendas por Vendedor
* Vendas Por Produto
* Estoque Inicial e Final

# 8. Dashboard PIB #
**Análise da Evolução do PIB ao Longo dos Anos:** 
* Evolução do PIB ao Longo Anos de Maneira Dinâmica
